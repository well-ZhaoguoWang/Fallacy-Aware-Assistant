if (!window.__MOD_CONTENT_EXIST) {
  ...
  let bubbleEl  = null;     // current bubble DOM
  let hideTimer = null;     // auto fade-out timer
  let progressBar = null;   // progress bar element
  let progressText = null;  // progress text element
  let progressDetails = null; // progress details element

  /* ---------- Background messages ---------- */
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.type === "MODERATE_PENDING") {
      showPending("⏳ Initializing detection..."); // single
    } else if (msg.type === "MODERATE_PENDING_BATCH") {
      showPending("⏳ Initializing batch analysis...");
    } else if (msg.type === "MODERATE_PROGRESS") {
      updateProgress(msg.progress); // single progress
    } else if (msg.type === "MODERATE_BATCH_PROGRESS") {
      updateProgress(msg.progress); // batch progress
  ...
  function showPending(text) {
    ...
    scheduleHide(60_000); // allow more time
  }
  ...
  function updateProgress(progress) {
    ...
    // Adjust progress bar color by stage
  ...
  function renderResult(payload) {
    ...
    // Keep results visible a bit longer
    scheduleHide(20_000);
  }

  /* ---------- Create / replace bubble ---------- */
  function resetBubble(text, color) {
    ...
    /* Insert into document, then position */
    ...
    /* Vertical position: above selection or 60px from top */
